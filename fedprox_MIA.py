...
def collect_fixed_samples(client_loader, num_samples):
    samples = []
    samples_per_batch = 2  # 每个 batch 取几个样本(因为正反例各抽200个，所以这里参数取2足够)
...
def run(args):
    ...
    criterion = nn.CrossEntropyLoss()
    result_path = os.path.join("result", "fedprox_MIA", f"{args.dataset}_{args.partition}.jsonl")
    ...
    for round_idx in range(args.global_rounds):
        ...
        if attack_client_id in selected_ids:
            ...
            print(f"      Min: {pos_stats['min']:.4f}, Q1: {pos_stats['q1']:.4f}, Median: {pos_stats['median']:.4f}, Q3: {pos_stats['q3']:.4f}, Max: {pos_stats['max']:.4f}")
            print(f"Neg Scores – Mean: {neg_stats['mean']:.4f}, Std: {neg_stats['std']:.4f}")
            print(f"      Min: {neg_stats['min']:.4f}, Q1: {neg_stats['q1']:.4f}, Median: {neg_stats['median']:.4f}, Q3: {neg_stats['q3']:.4f}, Max: {neg_stats['max']:.4f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="FedProx with Membership Inference Attack (Scheme A)")
    ...
    os.makedirs(os.path.join("result", "fedprox_MIA"), exist_ok=True)
    run(args)